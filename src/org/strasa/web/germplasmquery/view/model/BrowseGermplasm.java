...
public class BrowseGermplasm
{
	...
	private String ilNameSearch; // adding by QIN MAO
	private List<IntrogressionLine> lstIntrogressionLine = new ArrayList<IntrogressionLine>(); // adding by QIN MAO
	private HashMap<String, Object> searchIntrogressionLineParams = new HashMap<String, Object>(); // adding by QIN MAO
	private Integer userID = SecurityUtil.getDbUser().getId(); //adding by QIN MAO
...
//			adding by QIN MAO
			component.getFellow("defaultGermplasmDisplay").setVisible(true);
			component.getFellow("defaultIntrogressionLineDisplay").setVisible(
			        false);
...
//			adding by QIN MAO
		...
		} else
		{
			...
			component.getFellow("keyCharacteristicsOptions").setVisible(false);
			component.getFellow("resetKeyCharId").setVisible(false);
			if (selectedGermplasmType != null
			        && selectedGermplasmType.getGermplasmtype().equals("IL")) // adding by QIN MAO
			{
				component.getFellow("defaultGermplasmDisplay")
				        .setVisible(false);
				component.getFellow("defaultIntrogressionLineDisplay")
				        .setVisible(true);
				component.getFellow("searchIntrogresionLineOption").setVisible(
				        true);
			} else
	        ...
	        @ContextParam(ContextType.VIEW) Component view)
	{
		...
		} else if (cmbSearchKey.getValue().equals("Type"))
		{
			...
			{
//				modify by QIN MAO, adding one checking condition whether user choose IL germplasmtype, if yes run other search logic else by their their original method
				if (selectedGermplasmType.getGermplasmtype().equals("IL"))
				{
					searchIntrogressionLineParams.clear();
//					if(ilNameSearch != null)
//						searchIntrogressionLineParams.put("IntrogressionLineName", ilNameSearch.toString());
					if(segmentNumber != null)
						searchIntrogressionLineParams.put("SegmentNumber", segmentNumber);
					if(recurrentParent != null)
						searchIntrogressionLineParams.put("RecurrentParent", recurrentParent);
					if(donorParent != null)
						searchIntrogressionLineParams.put("DonorParent", donorParent);
					searchIntrogressionLineParams.put("UserId", this.getUserID());
					IntrogressionLineManagerImpl ilManagerImpl = new IntrogressionLineManagerImpl();
					lstIntrogressionLine.clear();
					lstIntrogressionLine = ilManagerImpl.getIntrogressionLineBySearchParams(searchIntrogressionLineParams);
					component.getFellow("resetKeyCharId").setVisible(true);
				} else 
				{
//					original
...
//		modfity by QIN MAO, adding on checking condition whether user choose IL germplasmtype, if yes update the searchResultLabel by lstIntrogressionLine, else  count by their original method
		if(selectedGermplasmType != null && selectedGermplasmType.getGermplasmtype().equals("IL"))
		{
			searchResultLabel = "Search Result:  " + this.lstIntrogressionLine.size()
			        + "  row(s) returned";
		} else
		{
	        ...
	        @ContextParam(ContextType.VIEW) Component view)
	{
		...
		germplasmList.clear();
		searchResultLabel = "";
		lstIntrogressionLine.clear(); // adding by QIN MAO
		component.getFellow("resetKeyCharId").setVisible(false); // adding by QIN MAO
		this.setIlNameSearch(null);
		this.setSegmentNumber(null);
		this.setRecurrentParent(null);
	}
...
//		segmentSearchTab.setSelected(true);
	...
	public String getIlNameSearch()
	{
		return ilNameSearch;
	}
	
	public void setIlNameSearch(String ilNameSearch)
	{
		this.ilNameSearch = ilNameSearch;
	}
	...
	public HashMap<String, Object> getSearchIntrogressionLineParams()
	{
		return searchIntrogressionLineParams;
	}

	public void setSearchIntrogressionLineParams(
	        HashMap<String, Object> searchIntrogressionLineParams)
	{
		this.searchIntrogressionLineParams = searchIntrogressionLineParams;
	}
